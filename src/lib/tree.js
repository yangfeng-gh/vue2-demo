...
export function transformPrivilegeTree2(treeData) {
  ...
  return treeData.map(item => {
    ...
    _item.type = item.type.value
    if (item.children && item.children.length) {
      _item.children = transformPrivilegeTree2(item.children)
      _item.checkedChildren = _item.children
        .filter(child => child.checked || child.indeterminate)
        .map(child => child.id)
      _item.checked = !!(_item.checkedChildren.length && _item.children.length === _item.checkedChildren.length)
      _item.indeterminate =
        (_item.checkedChildren.length && _item.checkedChildren.length < item.children.length) ||
        _item.children.some(child => child.indeterminate)
    } else {
